  ...
  def transactions_last_n_months(last_n_months=1)
	...
	def extract_balance_from_transactions
		...
		self.transactions.find(:all).sort_by(&:date).each do |transaction|
			...
			if !(self.bank_balances.find(:all, :conditions => {:datetime => transaction.date.to_datetime.advance(:seconds => -1)..transaction.date.to_datetime.advance(:seconds => 1)}).count > 0) && !(last_date === transaction.date.to_datetime)
