class CommentsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @comment.save
		    CommentMail.create(:body => @comment.body, :name => @comment.name, :email => @comment.email, :partner_email => (@post.partner.name.match(/\w+/).to_s.downcase + "@dreamyourweb.nl"), :post_title => @post.title).deliver
